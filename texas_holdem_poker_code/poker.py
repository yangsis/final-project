...
import sys
#citation: https://github.com/omarshammas/pyPoker-Texas-HoldEm
...
class Poker:
    ...
    #------------------Constructor-----------------
    #===============================================
    def __init__(self, number_of_players,money_of_each_player, debug = False):
        self.deck = deck()
        if number_of_players < 2 or number_of_players > 10:
            sys.exit("*** ERROR ***: Invalid number of players. It must be between 2 and 10.")
        if    money_of_each_player < 10:
            sys.exit("*** ERROR ***: the money of each player must be more than 10.")
        self.number_of_players = number_of_players
        self.money_of_each_player=money_of_each_player
    ...
    def change_num_of_players(self, num):
        ...
        if self.number_of_players < 2 or self.number_of_players > 10:
            sys.exit("*** ERROR ***: Invalid number of players. It must be between 2 and 10.")
    ...
    #------------------Distribute-------------------
    #===============================================
    def distribute(self): #inplay=[ [],[],], each[] means different cards one player gets
    ...
    def determine_score(self, community_cards, players_hands):
        ...
        for hand in players_hands:
            overall = self.score(hand)  #overall = [score, kicker]
            results.append([overall[0], overall[1]])  # Stores the results,results=[[score, kicker],[score, kicker],..]
        return results
    ...
    def determine_winner(self, results):
        #    results=[[score, kicker],[score, kicker],..]
        ...
        for r in results: #r=[score, kicker], r[0] = score
            if r[0] == high:
                kicker[counter] = r[1]
                #kicker[0]= {0:kicker,1:kicker,...} some the key is player's number, the value is its kicker
